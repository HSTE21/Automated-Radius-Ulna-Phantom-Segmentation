...
def main():
    ...
    results_file = os.path.join(script_dir, 'optuna_results_rw.csv')
    ...
    print(f"Applying Random Walker (slice by slice) with beta={beta}, t_marker_bg={t_marker_bg}, t_marker_fg={t_marker_fg}")
    ...
    plt.title(f'Random Walker (Dice={dice:.4f})')
    plt.savefig(os.path.join(script_dir, 'final_segmentation_rw.png'))
    
    nib.save(nib.Nifti1Image(seg.astype(np.float32), np.eye(4)), 
             os.path.join(script_dir, 'final_bone_mask_rw.nii'))
