...
def objective(trial):
    # Parameters for Sauvola
    window_size = trial.suggest_int('window_size', 5, 51, step=2)
    ...
    # In previous runs, bone was low intensity (t_high=79). 
    # Let's try both polarities or assume low.
    # Actually, let's suggest the polarity.
    polarity = trial.suggest_categorical('polarity', ['low', 'high'])
...
study.optimize(objective, n_trials=400, n_jobs=-1) # Sauvola is slower, so fewer trials

results = [
    {
        'trial': t.number,
        'window_size': t.params['window_size'],
        'k': t.params['k'],
        'polarity': t.params['polarity'],
...
df.to_csv(os.path.join(script_dir, 'optuna_results_local.csv'), index=False)

top_results = df.head(10)
top_results.to_csv(os.path.join(script_dir, 'top_optuna_results_local.csv'), index=False)

latex_table = top_results.to_latex(
    index=False,
    caption='Top 10 parameter configurations for local thresholding',
    label='tab:local_top',
    float_format='%.4f'
)
